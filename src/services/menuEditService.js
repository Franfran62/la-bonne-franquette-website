...
const getCategories = (data) => {
    ...
    data.map((e) => {
        if (e["categoryType"] === "category") {
            categories.push(
                new Category(e["id"], e["name"], [], e["categoryType"])
            );
    ...
    try {
        ...
        switch (type) {
            case MenuElements.ADDON: {
                response.data.map((e) => {
                    result.push(
                        new Addon(e["id"], e["name"], e["price"], e["VATRate"])
                    );
                });
                return result;
            }
            ...
            case MenuElements.SUBCATEGORY: {
                return getCategories(response.data).filter(
                    (e) => e instanceof SubCategory
                );
            }
            ...
            case MenuElements.PAYMENT_TYPE: {
                response.data.map((e) => {
                    result.push(
                        new PaymentType(e["id"], e["name"], e["isEnable"])
                    );
                });
                return result;
            }
